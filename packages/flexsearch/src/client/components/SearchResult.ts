...
import {
  flexsearchClientCustomFiledConfig,
  flexsearchLocales,
} from "../define.js";
  ...
  setup(props, { emit }) {
    ...
    const getDisplay = (matchedItem: MatchedItem): (VNode | string)[] => {
      if (matchedItem.type === "customField") {
        const formatterConfig =
          flexsearchClientCustomFiledConfig[matchedItem.index] || "$content";
    ...
    const renderSearchQueryHistory = (): VNode | null =>
      enableQueryHistory
        ? h(
            "ul",
            { class: "flexsearch-result-list" },
            h("li", { class: "flexsearch-result-list-item" }, [
              h(
                "div",
                { class: "flexsearch-result-title" },
              ...
              queryHistory.value.map((item, historyIndex) =>
                h(
                  ...
                  [
                    h(HistoryIcon, {
                      class: "flexsearch-result-type",
                    }),
                    h("div", { class: "flexsearch-result-content" }, item),
                    h("button", {
                      class: "flexsearch-remove-icon",
              ...
              resultHistory.value.map((item, historyIndex) =>
                h(
                  ...
                  () => [
                    ...
                    h("div", { class: "flexsearch-result-content" }, [
                      item.header
    ...
    watch(
      ...
      () => {
        document
          .querySelector(
            ".flexsearch-result-list-item.active .flexsearch-result-item.active",
          )
