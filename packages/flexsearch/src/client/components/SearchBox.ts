...
import { flexsearchHotKeys, flexsearchLocales } from "../define.js";
import { isFocusingTextControl, isKeyMatched } from "../utils/index.js";

import "../styles/search-box.scss";

const primaryKey = flexsearchHotKeys[0];
  ...
  setup() {
    ...
    return (): (VNode | null)[] => [
      h(
        ...
        [
          h(SearchIcon),
          h("div", { class: "flexsearch-placeholder" }, locale.value.search),
          controlKeys.value
            ? h(
                "div",
                { class: "flexsearch-key-hints" },
                controlKeys.value.map((key) =>
                  h("kbd", { class: "flexsearch-key" }, key),
                ),
              )
            : null,
        ],
